...
void Overlay::createPipe() {
	...
	if (! qlsServer->listen(pipepath)) {
        QMessageBox::warning(NULL, QLatin1String("Bubbles"), tr("Failed to create communication with overlay at %2: %1. No overlay will be available.").arg(Qt::escape(qlsServer->errorString()), Qt::escape(pipepath)), QMessageBox::Ok, QMessageBox::NoButton);
	} else {
